...
import { AlertCircle, AlertTriangle, Info, Sparkles, BrainCircuit, History, Trash2, Bot } from 'lucide-react';
...
export default function AlertPanel({ alerts, devices, connections, onVisualizeRca }: AlertPanelProps) {
                    ...
                    <div className="flex-1">
                      ...
                      {rootCauseInsight ? (
                        ...
                        <div className="mt-3">
                          <button
                            onClick={async () => {
                              if (onVisualizeRca && connections) {
                                const alertDevice = devices.find(d => d.name === alert.device || d.id === alert.device);
                                const rootId = alertDevice?.id || devices[0].id;
                                const affected = new Set<string>();
                                connections.forEach(c => {
                                  if (c.source === rootId) {
                                    affected.add(c.target);
                                    connections.forEach(hop => { if (hop.source === c.target) affected.add(hop.target) });
                                  }
                                });
                                onVisualizeRca(rootId, Array.from(affected));
                              } else {
                                handleAnalyze(alert.id, alert.device);
                              }
                            }}
                            ...
                            <div className="flex items-center justify-center gap-2">
                              {analyzingIds.has(alert.id) ? (
                                ...
                                <>
                                  ...
                                  <span className="text-xs font-bold text-slate-300 group-hover:text-purple-300 transition-colors">Route to RCA Engine (3D)</span>
                                </>
                              )}
                            </div>
                            {analyzingIds.has(alert.id) && (
